...
for game_id in consolidated_defined_actions['game_id'].unique():
    game_actions = consolidated_defined_actions[consolidated_defined_actions['game_id'] == game_id]
    
    # Get the first team_id (home team)
    home_team_id = game_actions['team_id'].iloc[0]
    
    # Get the next different team_id (away team)
    away_team_id = game_actions['team_id'].loc[game_actions['team_id'] != home_team_id].iloc[0]
    
    # Get team names from consolidated_teams
    home_team_name = consolidated_teams.loc[consolidated_teams['team_id'] == home_team_id, 'team_name'].values[0]
    away_team_name = consolidated_teams.loc[consolidated_teams['team_id'] == away_team_id, 'team_name'].values[0]
    
    # Append the game data to the list
    game_data.append({'home_team': home_team_name, 'away_team': away_team_name, 'game_id': game_id})

# Create a new DataFrame from the game data
game_data_df = pd.DataFrame(game_data)

common_game_ids = consolidated_defined_actions['game_id'].unique()

# Filter rows in game_data_df where game_id is in common_game_ids
filtered_df_games = game_data_df[game_data_df['game_id'].isin(common_game_ids)]
...
match_info = game_data_df.loc[game_data_df['game_id'] == selected_game_id]
